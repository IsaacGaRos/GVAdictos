#!/usr/bin/env python3
"""Migrate search feature tables to the database."""
...
def main():
    ...
    try:
        missing = missing_search_tables(conn)

        if not missing:
            print("[OK] All search feature tables already exist.")
            return 0
        ...
        print("Creating search feature tables...")
        ensure_search_tables(conn)
        print("[OK] Search feature tables created successfully.")

        missing_after = missing_search_tables(conn)
        if missing_after:
            print(f"[ERROR] Some tables still missing: {missing_after}")
            return 1

        print("[OK] All search feature tables verified.")
        return 0
